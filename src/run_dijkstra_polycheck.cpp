...
int main(int argc, char*argv[]){
	...
	try{
		...
		if(argc != 10){
			cerr << argv[0] << " first_out_file head_file weight_file lat_file lon_file polygons source_file target_file distance_file" << endl;
			return 1;
		}else{
		...
		for(unsigned i=0; i<query_count; ++i){
			...
			while(!dij.is_finished()){
				auto x = dij.settle(AvoidPolygonsGetWeight(weight, tail, head, lat, lon, polys)).node;
				if(x == target[i])
					break;
			}
